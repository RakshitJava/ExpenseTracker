...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    switch ($reportType) {
        case 'datewise':
            $query = "SELECT DATE(expensedate) AS period, expensecategory, SUM(expense) AS totalExpense FROM expenses WHERE user_id = '$userid' AND expensedate BETWEEN '$startDate' AND '$endDate' GROUP BY period, expensecategory";
            $tableHeader = 'Date';
            $periodDates = generateDateRange($startDate, $endDate);
            break;
        case 'monthwise':
            $query = "SELECT YEAR(expensedate) AS year, MONTH(expensedate) AS month, expensecategory, SUM(expense) AS totalExpense FROM expenses WHERE user_id = '$userid' AND expensedate BETWEEN '$startDate' AND '$endDate' GROUP BY year, month, expensecategory";
            $tableHeader = 'Year-Month';
            $periodDates = generateMonthYearRange($startDate, $endDate);
            break;
        case 'yearwise':
            $query = "SELECT YEAR(expensedate) AS year, expensecategory, SUM(expense) AS totalExpense FROM expenses WHERE user_id = '$userid' AND expensedate BETWEEN '$startDate' AND '$endDate' GROUP BY year, expensecategory";
            $tableHeader = 'Year';
                            ...
                            foreach ($periodDates as $periodDate) {
                                ...
                                while ($row = mysqli_fetch_array($exp_fetched)) {
                                    if ($reportType == 'datewise' && $row['period'] == $periodDate) {
                                        ...
                                        echo '<td class="text-center">' . $row['expensecategory'] . '</td>';
                                        echo '<td class="text-center">' . $row['totalExpense'] . '</td>';
                                        echo '</tr>';
                                        $count++;
                                    }
                                    elseif ($reportType == 'monthwise' && $row['period'] == $periodDate) {
                                        $totalAmount += $row['totalExpense'];
                                        echo '<tr>';
                                        echo '<td class="text-center">' . $count . '</td>';
                                        echo '<td class="text-center">' . $periodDate . '</td>';
                                    ...
                                    elseif ($reportType == 'yearwise' && $row['period'] == $periodDate) {
                                        $totalAmount += $row['totalExpense'];
                                        echo '<tr>';
                                        echo '<td class="text-center">' . $count . '</td>';
                                        echo '<td class="text-center">' . $periodDate . '</td>';
                                    ...
                                    // Similarly for monthwise and yearwise
                                }
                            }
